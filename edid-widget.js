...
(function() {
  ...
  // 2) Inject styles scoped to this widget
  ...
  style.textContent = `
    ...
    #edid-calculator input, select, #edid-gen-btn { width:100%; padding:0.5rem; font-size:1rem; box-sizing:border-box; }
    #edid-gen-btn { margin-top:1rem; cursor:pointer; }
    #edid-results { margin-top:1.5rem; background:#fff; padding:1rem; border-radius:4px; display:none; line-height:1.4; }
    #edid-results span { font-weight:bold; }
    #edid-download { display:block; margin-top:1rem; text-align:center; }
  `;
  ...
  container.innerHTML += `
    ...
    <div id="edid-results">
      ...
      <div><span>Data Rate (24 bpp):</span> <span id="edid-datarate"></span> Gbps</div>
  ...
  // 4) Hook up logic
  function generate() {
    const w     = parseInt(document.getElementById('edid-width').value);
    const h     = parseInt(document.getElementById('edid-height').value);
    const r     = parseFloat(document.getElementById('edid-refresh').value);
    ...
    let cable;
    if      (dr <= 4.95)  cable = "HDMI 1.2 / SL-DVI";
    else if (dr <= 10.2)  cable = "HDMI 1.4 / DL-DVI";
    else if (dr <= 18)    cable = "HDMI 2.0 / DP 1.2";
    else if (dr <= 25.92) cable = "DP 1.4";
    else                  cable = "DP 2.0+";
    ...
    document.getElementById('edid-datarate').textContent = dr.toFixed(2);
    ...
    edid.set([0x4C,0x2D], 8);       // manufacturer
    edid[16] = 1; edid[17] = 4;      // version 1.4
    const d = 54;
    ...
    const link = document.getElementById('edid-download');
    link.href = url;
    link.download = `EDID_${w}x${h}_${r.toFixed(2)}.bin`;
  }
