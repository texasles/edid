// edid-widget.js
(function() {
  const hcReadyPromise = new Promise(resolve => {
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    const results = get('edid-results');

    get('edid-gen-btn').addEventListener('click', async () => {
      ...
      let signal = pclk < 165 ? 'Single Link' : (pclk < 330 ? 'Dual Link' : '4K');
      ...
      get('edid-signalType').textContent = signal;
      ...
      edid[16] = 1; edid[17] = 4;
      const D = 54;
      const clk10 = Math.round(pclk * 100);
      ...
      edid[D+8] = hFront & 0xFF; edid[D+9] = hSync & 0xFF;
      edid[D+10] = ((hSync >> 8) & 0x3) << 6 | ((hFront >> 8) & 0x3) << 4;
      edid[D+11] = vFront & 0xFF; edid[D+12] = vSync & 0xFF;
